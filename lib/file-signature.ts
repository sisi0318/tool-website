...
export function createBinaryFile(bytes: Uint8Array<ArrayBuffer>, name: string, modifiedAt?: number): File {
  return new File([bytes], name.split(/[\\/]/).pop() || "output.bin", { type: detectFileSignature(bytes).mime, ...(modifiedAt !== undefined ? { lastModified: modifiedAt } : {}) })
}
