...
export type BaseConverterErrorCode =
  | "baseOutOfRange"
  | "emptyInput"
  | "invalidInteger"
  | "invalidDigit"

export class BaseConverterError extends Error {
  constructor(
    readonly code: BaseConverterErrorCode,
    message: string,
    readonly details?: { base?: number; character?: string },
  ) {
    super(message)
    this.name = "BaseConverterError"
  }
}

function validateBase(base: number): void {
  if (!Number.isInteger(base) || base < 2 || base > RADIX_ALPHABET.length) {
    throw new BaseConverterError(
      "baseOutOfRange",
      `进制必须是 2 到 ${RADIX_ALPHABET.length} 之间的整数`,
      { base },
    )
  }
}
...
export function parseBigIntInBase(value: string, base: number): bigint {
  ...
  if (!compact) throw new BaseConverterError("emptyInput", "请输入要转换的整数")

  const negative = compact.startsWith("-")
  const digits = negative || compact.startsWith("+") ? compact.slice(1) : compact
  if (!digits) throw new BaseConverterError("invalidInteger", "请输入有效的整数")
